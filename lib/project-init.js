...
export function initProject(projectName, options = {}) {
    ...
    fs.writeFileSync(
        path.join(projectPath, 'markdown-to-pdf.config.json'),
...
This project was initialized with markdown-to-pdf.

## Getting Started

1. Edit your markdown files in the \`docs/\` directory
2. Configure authors in \`data/authors/\`
3. Customize the theme in \`markdown-to-pdf.config.json\`
...
\`\`\`bash
markdown-to-pdf convert docs/example.md -o output/my-document.pdf
\`\`\`

Generate with HTML output:
\`\`\`bash
markdown-to-pdf convert docs/example.md --html
\`\`\`
