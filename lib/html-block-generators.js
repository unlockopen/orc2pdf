...
export function injectAuthorsHtml(fileMetadata, markdown, config = {}) {
...
export function getFooter(fileMetadata, theme = {}) {
    // Read the footer file from theme
    const footerFile = theme.footerTemplate || path.join(theme.path || '', 'footer.html');
    if (!fs.existsSync(footerFile)) {
        return '<div style="font-size: 10px; text-align: center;"><span class="pageNumber"></span></div>';
    }
...
export function getHeader(fileMetadata, theme = {}) {
    // Read the header file from theme
    const headerFile = theme.headerTemplate || path.join(theme.path || '', 'header.html');
    if (!fs.existsSync(headerFile)) {
        return '<div></div>'; // Empty header if no template found
    }
...
export function getTitlePage(pageMetadata, theme = {}) {
    // Generate the title page HTML content
    const title = pageMetadata['title'] || '';
    const subtitle = pageMetadata['subtitle'] || '';
    
    const titlePageStylesheet = theme.titlePageStylesheet || 
                               path.join(theme.path || '', 'title-page.css');
    ...
    const htmlContent = `
        ...
        <head>
            <meta charset="utf-8">
            <title>${title}</title>
            <link rel="stylesheet" href="${titlePageStylesheet}">
        </head>
        <body>
