...
export async function loadConfig(projectDir, configPath = null) {
    ...
    const possibleConfigs = [
        configPath,
        path.join(projectDir, 'markdown-to-pdf.config.js'),
        path.join(projectDir, 'markdown-to-pdf.config.json'),
        path.join(projectDir, '.markdown-to-pdf.json'),
        path.join(projectDir, 'package.json') // Look for markdown-to-pdf section
    ].filter(Boolean);
        ...
        if (fs.existsSync(configFile)) {
            try {
                ...
                if (configFile.endsWith('.js')) {
                    ...
                    loadedConfig = packageJson['markdown-to-pdf'] || {};
                } else {
