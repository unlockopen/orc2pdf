...
export async function generatePdf(inputFile, options = {}) {
    ...
    // Determine template path - try theme's template first, fall back to default
    const templatePath = path.join(theme.path || '', 'main-content.html');
    const actualTemplatePath = fs.existsSync(templatePath) ? templatePath : null;
    
    let htmlContent = await mdToHtml(processedMarkdown, theme.mainContentStylesheet, pageMetadata, {}, [], actualTemplatePath);
