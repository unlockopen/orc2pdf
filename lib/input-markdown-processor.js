...
export function processInputMarkdown(inputFilePath, config = {}) {
    ...
    getAuthorData(pageMetadata, messages, config);
...
function authorPicUrl(authorID) {
    const authorPicPath = `data/authors/pictures/${authorID}`;
...
function getAuthorData(pageMetadata, messages, config = {}) {
    ...
    const authorTemplate = config.authors?.template;
    if (!authorTemplate || !fs.existsSync(authorTemplate)) {
        addWarning(messages, '⚠️ Author template file not found, skipping author processing.');
        return;
    }
    
    const templateAuthorContent = fs.readFileSync(authorTemplate, 'utf8');
    ...
    pageMetadata.authors.forEach(({ name, email }) => {
        ...
        const authorFilePath = `${config.authors?.directory || 'data/authors'}/${authorId}.yaml`;
